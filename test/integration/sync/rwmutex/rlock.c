...
void test_rwmutex_concurrent_readers(void) {
    ...
    int spins = 0;
    while (atomic_load_explicit(&completed, memory_order_acquire) < R) {
        nanosleep(&ts, NULL);
        spins++;
        if (spins > 5000) { /* ~5s */
    ...
    TEST_ASSERT_EQUAL_INT(R, atomic_load(&completed));
...
int main(void) {
    ...
    RUN_TEST(test_rwmutex_concurrent_readers);
